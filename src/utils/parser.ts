...
/** 常见章节标题正则 */
const CHAPTER_RE =
  /^[\s]*(?:第[零一二三四五六七八九十百千万\d]+[章节回卷集部篇]|chapter\s*\d+|序[章言]?|楔子|尾声|番外|引子|前言|后记|【\s*(?:番外|序[章言]?|楔子|尾声|引子|前言|后记)\s*】)\s*.*/i;

/** 番外标题：仅按“番外”前缀匹配，后续内容（含编号）自然兼容 */
const FANWAI_RE = /^[\s]*(?:【\s*)?番外(?:\s*】)?\s*.*/i;
...
export function parseTxt(text: string, fileName: string): ParsedBook {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      (CHAPTER_RE.test(trimmedLine) || FANWAI_RE.test(trimmedLine))
    ) {
      // 遇到新章节标题
...
export async function parseEpub(data: ArrayBuffer, fileName: string): Promise<ParsedBook> {
  ...
  const author =
    opfDoc.querySelector('metadata > *|creator, metadata creator')?.textContent?.trim() ?? '未知';
  ...
  for (const idref of spineRefs) {
    ...
    // 尝试提取章节标题
    const heading = body.querySelector('h1, h2, h3, h4')?.textContent?.trim() ?? '';
...
/** 根据文件扩展名解析书籍 */
export async function parseBook(fileData: ArrayBuffer, fileName: string): Promise<ParsedBook> {
